...
class MuridController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // 👇 TAMBAHKAN INI - Ambil paket list untuk filter
        $paketList = [];
        foreach($murids as $m) {
        ...
        return view('dashboard.shared.kelola-murid.kelola-murid', [
            'role' => $role,
            'murids' => $murids,
            'paketList' => $paketList,  // 👈 KIRIM KE VIEW
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $dataMurid = $request->only([
                'nama_lengkap', 'asal_sekolah', 'alamat', 'no_hp',
                'nama_orang_tua', 'no_hp_orang_tua', 'tahun_masuk',
            ]);
            ...
            if ($kelas) $kelas->increment('jumlah_murid');
            
            DB::commit();
            
            $role = str_contains($request->url(), 'superadmin') ? 'superadmin' : 'admin';
            
            // 👇 Redirect dengan JSON untuk AJAX
    ...
    // Search murid untuk autocomplete
