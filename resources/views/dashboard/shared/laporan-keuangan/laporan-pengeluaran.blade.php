...
<div style="width: 100%;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; gap: 12px; flex-wrap: wrap;">
        <form method="GET" action="{{ route($role . '.laporan-keuangan.pengeluaran') }}" style="display: flex; gap: 12px; flex-wrap: wrap; flex: 1;">
            ...
            @if($filterBulan || $filterTahun || $filterPeriode)
                <a href="{{ route($role . '.laporan-keuangan.pengeluaran') }}" style="padding: 10px 15px; background: #F3F4F6; color: #374151; border-radius: 12px; text-decoration: none; font-size: 13px; display: flex; align-items: center; gap: 5px;">
    ...
    <div style="background: white; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.08); border: 1px solid #F3F4F6;">
            ...
            <table class="table-pengeluaran" style="width: 100%; border-collapse: collapse; white-space: nowrap;">
                ...
                <tbody style="color: #374151;">
                    ...
                    <tr>
                        ...
                        <td style="text-align: right; font-weight: 600; color: #DC2626;">Rp {{ number_format($t->jumlah, 0, ',', '.') }}</td>
                    </tr>
                    @empty
        ...
        <div style="display: flex; justify-content: space-between; padding: 14px 20px; background: #F9FAFB; border-top: 2px solid #E5E7EB; font-weight: 700; font-size: 14px;">
            <span>TOTAL</span>
            <span style="color: #DC2626;">Rp {{ number_format($total, 0, ',', '.') }}</span>
        </div>
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; padding: 10px 20px; background: #F9FAFB; border-top: 1px solid #E5E7EB;">
            ...
            <div style="display: flex; gap: 5px;">
                ...
                @else
                    <a href="{{ request()->fullUrlWithQuery(['page' => 1]) }}" style="width: 35px; height: 35px; border-radius: 8px; border: 1px solid #D74E4E; background: white; color: #D74E4E; display: flex; align-items: center; justify-content: center; text-decoration: none;"><i class="fas fa-angle-double-left"></i></a>
                    <a href="{{ request()->fullUrlWithQuery(['page' => $transaksi->currentPage() - 1]) }}" style="width: 35px; height: 35px; border-radius: 8px; border: 1px solid #D74E4E; background: white; color: #D74E4E; display: flex; align-items: center; justify-content: center; text-decoration: none;"><i class="fas fa-angle-left"></i></a>
                @endif
                ...
                @for ($i = $start; $i <= $end; $i++)
                    @if ($i == $transaksi->currentPage())
                        <button style="width: 35px; height: 35px; border-radius: 8px; background: #D74E4E; color: white; border: none; font-weight: 600; cursor: pointer;">{{ $i }}</button>
                    @else
                        <a href="{{ request()->fullUrlWithQuery(['page' => $i]) }}" style="width: 35px; height: 35px; border-radius: 8px; border: 1px solid #D74E4E; background: white; color: #D74E4E; display: flex; align-items: center; justify-content: center; text-decoration: none;">{{ $i }}</a>
                    @endif
                @endfor

                @if ($transaksi->hasMorePages())
                    <a href="{{ request()->fullUrlWithQuery(['page' => $transaksi->currentPage() + 1]) }}" style="width: 35px; height: 35px; border-radius: 8px; border: 1px solid #D74E4E; background: white; color: #D74E4E; display: flex; align-items: center; justify-content: center; text-decoration: none;"><i class="fas fa-angle-right"></i></a>
                @else
