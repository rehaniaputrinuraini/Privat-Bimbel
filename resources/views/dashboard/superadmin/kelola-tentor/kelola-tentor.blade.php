                ...
                <tbody id="tableBody" style="color: #374151;">
                    ...
                    <tr id="{{ $rowId }}" style="border-bottom: 1px solid #F3F4F6;">
                        ...
                        <td style="padding: 15px; text-align: center;">
                            <button onclick="toggleDetail('{{ $rowId }}', '{{ $hashId }}')" 
                        ...
                        <td style="padding: 15px; white-space: nowrap;">
                            <div style="display: flex; gap: 3px; justify-content: center; flex-wrap: nowrap;">
                                <button onclick="bukaModalEdit('{{ $hashId }}')" style="background: #5EB37E; color: white; padding: 4px 7px; border-radius: 5px; border: none; cursor: pointer; font-size: 10px; white-space: nowrap;"><i class="far fa-edit"></i> Edit</button>
                                <button onclick="bukaModalPassword('{{ $hashId }}', '{{ $nama }}')" style="background: #F59E0B; color: white; padding: 4px 7px; border-radius: 5px; border: none; cursor: pointer; font-size: 10px; white-space: nowrap;"><i class="fas fa-key"></i> Pass</button>
                                <button onclick="bukaModalHapus('{{ $hashId }}', '{{ $nama }}')" style="background: #E35D5D; color: white; padding: 4px 7px; border-radius: 5px; border: none; cursor: pointer; font-size: 10px; white-space: nowrap;"><i class="fas fa-trash"></i> Hapus</button>
                                <button onclick="bukaModalToggle('{{ $hashId }}', '{{ $nama }}', '{{ $status }}')" style="background: {{ $status == 1 ? '#EF4444' : '#10B981' }}; color: white; padding: 4px 7px; border-radius: 5px; border: none; cursor: pointer; font-size: 10px; white-space: nowrap;"><i class="fas {{ $status == 1 ? 'fa-ban' : 'fa-check' }}"></i> {{ $status == 1 ? 'Nonaktif' : 'Aktifkan' }}</button>
                            </div>
                        </td>
                    </tr>
                    @empty
...
<script>
    ...
    function toggleDetail(rowId, hashId) {
        ...
        if (existingDetail) {
            ...
            fetch("{{ route('superadmin.kelola-tentor.detail', '') }}/" + hashId)
                .then(r => r.json())
                .then(data => {
    ...
    function bukaModalEdit(hashId) {
        fetch("{{ route('superadmin.kelola-tentor.edit', '') }}/" + hashId)
            .then(r => r.text())
            .then(html => {
                document.getElementById('modalContent').innerHTML = html;
                document.getElementById('modalForm').style.display = 'flex';
    ...
    function bukaModalPassword(hashId, nama) {
        ...
        setTimeout(() => {
            ...
            document.getElementById('formPassword').addEventListener('submit', function(e) {
                ...
                fetch("{{ route('superadmin.kelola-tentor.updatePassword', '') }}/" + hashId, { 
                    method: 'PUT', 
                    headers: { 
    ...
    function bukaModalToggle(hashId, nama, status) {
        document.getElementById('formToggle').action = "{{ route('superadmin.kelola-tentor.toggleStatus', '') }}/" + hashId;
        if (status == 1) {
    ...
    function bukaModalHapus(hashId, nama) {
        document.getElementById('formHapus').action = "{{ route('superadmin.kelola-tentor.destroy', '') }}/" + hashId;
