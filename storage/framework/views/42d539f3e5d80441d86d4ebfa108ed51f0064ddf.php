    ...
    <div style="background: #F9FAFB; border-radius: 12px; padding: 20px; margin-bottom: 20px;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
            ...
            <div style="text-align: center; padding: 12px; background: white; border-radius: 8px; border: 1px solid #E5E7EB;">
                ...
                <?php if($statusTagihan == 'Lunas' || $statusTagihan == 'Uang Muka'): ?>
    ...
    <?php if($allTagihan->count() > 0): ?>
        <div style="overflow-x: auto; border-radius: 12px; border: 1px solid #E5E7EB;">
            <table style="width: 100%; border-collapse: collapse; font-size: 13px; font-family: 'Poppins', sans-serif;">
                ...
                <tbody>
                    <?php $__currentLoopData = $allTagihan; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <tr style="border-bottom: 1px solid #F3F4F6; <?php echo e($i % 2 == 0 ? 'background: #FAFAFA;' : ''); ?>">
                        ...
                        <td style="padding: 10px 14px; font-weight: 500;"><?php echo e($item->keterangan); ?></td>
                        <td style="padding: 10px 14px;"><?php echo e($item->tanggal_bayar ?? '-'); ?></td>
                        <td style="padding: 10px 14px;"><?php echo e($item->jenis_pembayaran ?? '-'); ?></td>
                        <td style="padding: 10px 14px; text-align: right; font-weight: 600; <?php echo e($item->status == 'Lunas' ? 'color: #10B981;' : ($item->status == 'Uang Muka' ? 'color: #F59E0B;' : 'color: #9CA3AF;')); ?>">
                            <?php echo e($item->jumlah ? 'Rp ' . number_format($item->jumlah, 0, ',', '.') : '-'); ?>

                        </td>
                        <td style="padding: 10px 14px; text-align: center;">
                            <?php if($item->status == 'Lunas'): ?>
                                <span style="padding:4px 10px;border-radius:20px;background:#D1FAE5;color:#065F46;font-size:11px;font-weight:600;">Lunas</span>
                            <?php elseif($item->status == 'Uang Muka'): ?>
