        ...
        <div style="display: flex; align-items: center; gap: 12px; flex: 1;">
            ...
            <select id="filterTahun" style="padding: 10px 12px; border-radius: 12px; border: 1px solid #E5E7EB; color: #374151; font-size: 13px; min-width: 160px; background: white; outline: none; cursor: pointer;">
                ...
                <?php $__currentLoopData = $tahunPeriodeList; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tahunPeriode): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($tahunPeriode); ?>" <?php echo e($tahunPeriode == ($periodeAktif->tahun_periode ?? '') ? 'selected' : ''); ?>><?php echo e($tahunPeriode); ?></option>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </select>
        </div>
                    ...
                    <tr style="border-bottom: 1px solid #F3F4F6; transition: 0.2s;" onmouseover="this.style.background='#F9FAFB'" onmouseout="this.style.background='transparent'">
                        ...
                        <td style="padding: 15px; text-align: center;">
                            <div style="display: flex; gap: 3px; justify-content: center; flex-wrap: nowrap;">
                                <button onclick="bukaModalEdit('<?php echo e(hash_id($m->id_murid)); ?>')" 
                                ...
                                <button onclick="bukaModalLanjutPeriode('<?php echo e(hash_id($m->id_murid)); ?>', '<?php echo e($m->nama_lengkap); ?>')" 
                                ...
                                <button type="button" onclick="bukaModalHapus('<?php echo e(hash_id($m->id_murid)); ?>', '<?php echo e($m->nama_lengkap); ?>')" 
...
<script>
    ...
    function bukaModalEdit(hashId) {
        fetch("<?php echo e(route($role . '.murid.edit', '')); ?>/" + hashId)
            .then(r => r.text())
            .then(html => {
                document.getElementById('modalContent').innerHTML = html;
                document.getElementById('modalForm').style.display = 'flex';
    ...
    function bukaModalHapus(hashId, nama) {
        document.getElementById('formHapus').action = "<?php echo e(route($role . '.murid.destroy', '')); ?>/" + hashId;
    ...
    function bukaModalLanjutPeriode(hashId, nama) {
        fetch("<?php echo e(route($role . '.murid.lanjut-periode-form', '')); ?>/" + hashId)
            .then(r => r.text())
            .then(html => {
