...
<div style="width: 100%;">
    ...
    <?php if(session('error')): ?>
        <div style="background: #FEE2E2; color: #991B1B; padding: 12px; border-radius: 10px; margin-bottom: 20px;"><?php echo e(session('error')); ?></div>
    <?php endif; ?>
    ...
    <div style="background: white; border-radius: 20px; overflow-x: auto; box-shadow: 0 4px 15px rgba(0,0,0,0.08);">
        <table style="width: 100%; border-collapse: collapse; font-size: 13px; white-space: nowrap; font-family: 'Poppins', sans-serif;">
            <thead><tr style="background: #F3E8FF;">
                ...
                <th style="padding: 15px; text-align: left; width: 200px;">Keperluan</th>
            ...
            <tbody id="tableBody">
                ...
                <tr style="border-bottom: 1px solid #F3F4F6;">
                    ...
                    <td style="padding: 15px; text-align: left; max-width: 200px; overflow: hidden; text-overflow: ellipsis;"><?php echo e($item->keperluan); ?></td>
                    ...
                    <td style="padding: 15px; text-align: center;">
                            <button type="button" onclick="bukaModalHapus('<?php echo e(hash_id($item->id)); ?>', '<?php echo e(addslashes($item->keperluan)); ?>')"    
                ...
                <tr><td colspan="7" style="padding: 50px; text-align: center; color: #9CA3AF;">Belum ada data pengeluaran</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
...
<div id="modalHapus" style="display: none; position: fixed; z-index: 9999; left: 0; top: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.4); backdrop-filter: blur(3px); align-items: center; justify-content: center;">
    <div style="background: white; padding: 25px; border-radius: 20px; width: 380px; text-align: center; box-shadow: 0 15px 30px rgba(0,0,0,0.15); font-family: 'Poppins', sans-serif;">
        ...
        <p style="color: #6B7280; font-size: 12px; margin: 8px 0 20px 0; line-height: 1.5;" id="pesanHapus">Apakah Anda yakin ingin menghapus data pengeluaran ini?</p>
...
<script>
    ...
    // =============================================
    function bukaModalHapus(id, keperluan) {
        // Set action form ke route hapus yang sesuai
        let url = "<?php echo e(route($role . '.pengeluaran.destroy', '')); ?>/" + id;
        document.getElementById('formHapus').action = url;
        document.getElementById('pesanHapus').innerHTML = `Apakah Anda <strong>benar-benar yakin</strong> ingin menghapus data pengeluaran untuk <strong>${keperluan}</strong>?<br><br><small style="color:#EF4444;">⚠️ <strong>PERINGATAN:</strong> Data akan dihapus <strong>secara permanen</strong> dari database.</small>`;
